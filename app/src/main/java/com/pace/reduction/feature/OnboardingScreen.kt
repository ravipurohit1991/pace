...
internal fun GuidedOnboardingScreen(initial: PlanSettings, onFinish: (PlanSettings) -> Unit) {
    ...
    // Onboarding runs before the app shell exists, so it is the one screen that has to keep itself
    ...
    Column(modifier = Modifier.fillMaxSize().safeDrawingPadding()) {
        LazyColumn(
            ...
            item {
                ...
                // Five steps of text gave no sense of how much was left; a bar answers that at a glance.
        ...
        // Back/Next stay put instead of scrolling away below the last field of a long step.
        StickyActionBar {
            Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                if (page > 0) {
                    OutlinedButton(
